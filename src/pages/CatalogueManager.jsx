  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }} className="animate-fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--text-main)', marginBottom: 4 }}>Catalogue & Brochures</h1>
      ...
      {showUpload && (
        <Card style={{ padding: 24, border: '1px solid rgba(168,137,68,0.3)', background: 'linear-gradient(to right, rgba(168,137,68,0.03), rgba(0,0,0,0))', animation: 'fadeIn 0.3s' }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-main)', marginBottom: 16 }}>Upload New Catalogue</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div>
              ...
              <input placeholder="e.g. Kitchen Designs 2026" style={{ width: '100%', padding: '10px 14px', background: 'var(--glass-light)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-main)', fontSize: 13, transition: 'all 0.2s', outline: 'none' }} onFocus={e => e.currentTarget.style.border = '1px solid var(--primary)'} onBlur={e => e.currentTarget.style.border = '1px solid var(--border)'} />
            </div>
            <div>
              <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>Type</label>
              <select style={{ width: '100%', padding: '10px 14px', background: 'var(--glass-light)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-main)', fontSize: 13, transition: 'all 0.2s', outline: 'none' }} onFocus={e => e.currentTarget.style.border = '1px solid var(--primary)'} onBlur={e => e.currentTarget.style.border = '1px solid var(--border)'}>
          ...
          <div style={{ padding: 32, border: '1px dashed var(--border)', borderRadius: 'var(--radius-lg)', background: 'linear-gradient(to bottom, var(--glass-light), transparent)', textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s' }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--primary)'; e.currentTarget.style.background = 'rgba(168,137,68,0.03)'; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'linear-gradient(to bottom, var(--glass-light), transparent)'; }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
        {catalogues.map(cat => (
          <Card key={cat.id} style={{ padding: 0, overflow: 'hidden', transition: 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.3s cubic-bezier(0.4, 0, 0.2, 1)' }}
            ...
            {/* Preview Header */}
            <div style={{ height: 160, background: 'radial-gradient(circle at top right, rgba(168,137,68,0.15) 0%, rgba(20,24,32,1) 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', borderBottom: '1px solid var(--glass-hover)' }}>
            ...
            <div style={{ padding: 16 }}>
              <h4 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-main)', marginBottom: 4 }}>{cat.name}</h4>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <Button variant="ghost" size="sm" style={{ color: '#F87171', border: 'solid1pxx #F87171' }}><Trash2 size={13} /></Button>
              </div>
            </div>
          </Card>
        ))}
        ...
        <Modal onClose={() => setSendModal(null)}>
          {sentSuccess ? (
            <div style={{ textAlign: 'center', padding: 24 }}>
              ...
              <h3 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-main)', marginBottom: 8 }}>Sent Successfully!</h3>
              <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>"{sendModal.name}" has been sent via {sendChannel}.</p>
            </div>
          ) : (
            <>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-main)', marginBottom: 4 }}>Send "{sendModal.name}"</h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  ...
                  <input value={sendTo} onChange={e => setSendTo(e.target.value)} placeholder="+91 98860 12345 or email@example.com" style={{ width: '100%', padding: '10px 14px', background: 'var(--glass-light)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-main)', fontSize: 13, transition: 'all 0.2s', outline: 'none' }} onFocus={e => e.currentTarget.style.border = '1px solid var(--primary)'} onBlur={e => e.currentTarget.style.border = '1px solid var(--border)'} />
                </div>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {sendModal.channels.map(ch => (
                      <button key={ch} onClick={() => setSendChannel(ch)} style={{ padding: '8px 16px', borderRadius: 'var(--radius-md)', fontSize: 13, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6, background: sendChannel === ch ? 'var(--primary-bg)' : 'var(--glass-light)', border: `1px solid ${sendChannel === ch ? 'var(--primary)' : 'var(--border)'}`, color: sendChannel === ch ? 'var(--primary-light)' : 'var(--text-muted)', cursor: 'pointer', transition: 'all 0.2s' }} onMouseEnter={e => { if (sendChannel !== ch) e.currentTarget.style.background = 'var(--glass-hover)'; }} onMouseLeave={e => { if (sendChannel !== ch) e.currentTarget.style.background = 'var(--glass-light)'; }}>
